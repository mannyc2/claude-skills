...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/progress_manager.py status
```
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/spaced_scheduler.py queue 10
```
...
Read `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/topic_graph.md` to understand:
...
**Adaptive Difficulty**:
- Query BKT mastery: `python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/knowledge_tracker.py get <topic_id>`
...
**NEVER provide direct instruction first.** Use Socratic questioning from `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/socratic_prompts.md`.
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/knowledge_tracker.py update <topic_id> <correct:true|false>
```

**Update SM-2** (Spaced Repetition):
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/spaced_scheduler.py review <topic_id> <quality:0-5>
```

**Check Cheat Sheet Suggestion**:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/cheat_sheet_builder.py check_suggest <topic_id>
```
...
- If user approves (y):
  ```bash
  python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/cheat_sheet_builder.py add <topic_id>
  ```
...
**Log Session**:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/progress_manager.py log_session
```
...
- `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/patterns/scheduling.md` (FCFS, SJF, RR, Priority, MLFQ, RM, EDF)
- `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/patterns/synchronization.md` (Peterson's, semaphores, monitors, classic problems)
- `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/patterns/memory-management.md` (Paging, segmentation, TLB, address translation)
- `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/patterns/page-replacement.md` (FIFO, LRU, Optimal, Clock, thrashing)
- `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/patterns/virtualization.md` (Type 0/1/2, trap-and-emulate, paravirtualization)
...
### Always Available
- Topic graph: `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/topic_graph.md`
- Socratic prompts: `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/socratic_prompts.md`
- Pattern guides: `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/references/patterns/*.md`
...
### Scripts
- BKT tracker: `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/knowledge_tracker.py`
- SM-2 scheduler: `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/spaced_scheduler.py`
- Progress manager: `${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/progress_manager.py`
...
> python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/spaced_scheduler.py queue 5
...
**Update**:
```bash
> python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/knowledge_tracker.py update cpu_scheduling_srtf true
Mastery: 58% → 72% (↑14%)

> python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/spaced_scheduler.py review cpu_scheduling_srtf 4
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/cheat_sheet_builder.py add <topic_id> [--note "custom note"]
```

**Remove topic**:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/cheat_sheet_builder.py remove <topic_id>
```

**View current cheat sheet items** (sorted by priority):
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/cheat_sheet_builder.py list
```

**Generate final exam cheat sheet**:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/cheat_sheet_builder.py generate [--max-topics 30]
```

**Check cheat sheet status**:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/cheat_sheet_builder.py status
```

**Test content extraction** (see what content exists for a topic):
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/os-tutor/scripts/content_extractor.py <topic_id>
```
