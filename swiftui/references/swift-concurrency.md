...
## Debugging: Thread Hops at `await`

With `-default-isolation=MainActor`, implicit isolation can cause unexpected thread hops:

```swift
// This struct is implicitly @MainActor isolated!
struct ShopifyAPI {
    func fetchProducts() async throws -> [Product] { ... }
}

actor StoreService: ModelActor {  // Background actor
    func sync() async {
        // BUG: This await HOPS TO MAIN THREAD because fetchProducts
        // is implicitly @MainActor. The sync blocks UI.
        let products = try await api.fetchProducts()
    }
}

// Fix: Add nonisolated to opt out
struct ShopifyAPI {
    nonisolated func fetchProducts() async throws -> [Product] { ... }
}
```

**Key insight**: Compiler warnings like "main actor-isolated ... called from nonisolated context" are telling you about thread hops. Pay attention to them.

**Debugging tip**: Log `Thread.isMainThread` before and after `await` to catch hops:

```swift
print("before await: isMain=\(Thread.isMainThread)")
let result = try await api.fetchProducts()
print("after await: isMain=\(Thread.isMainThread)")  // If this changed, you found the hop
```
