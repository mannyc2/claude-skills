...
**IMPORTANT**: When this skill is invoked, the working directory is already set to the `tempo-protocol/` directory. DO NOT use `cd tempo-protocol` commands.

**Fetching documentation:**
1. **ALWAYS check [references/docs-map.md](references/docs-map.md) first** to find the correct path
2. Use `uv run scripts/fetch_docs.py <path>` to retrieve docs
...
uv run scripts/fetch_docs.py /guide/payments

# ❌ Wrong - uses python instead of uv run
python scripts/fetch_docs.py /protocol/tip20/spec

# ❌ Wrong - tries to cd when already in tempo-protocol directory
...
# Filter by file type
uv run scripts/search_repo.py "transaction" --include "*.mdx,*.md" --max-matches 5
```
