...
cd /Users/chriscarroll/claude-skills/smart-cli-wrapper

# Test with sample command
python3 scripts/cli_interceptor.py test

# Execute specific command with compression
python3 scripts/cli_interceptor.py execute "kubectl get pods -A"

# With user intent for better compression
python3 scripts/cli_interceptor.py execute "kubectl get pods -A" --intent "Check which pods are failing"
```
...
python3 scripts/claude_compressor.py test

# Compress a file
python3 scripts/claude_compressor.py compress output.txt "kubectl get pods"
```
